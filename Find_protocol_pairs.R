...
newtable<-select(read.csv("~/Desktop/shark2"), Protocol,Source,Destination,SPort,DPort)
#структурируем данные(struct the data)
RESULT_TABLE<-data.table::data.table(count(group_by(newtable, Protocol,Source,SPort, Protocol,Destination,DPort)))
...
write.csv(RESULT_TABLE, file="RESULT2P.csv")
rm(newtable)
